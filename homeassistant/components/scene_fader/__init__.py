...
from homeassistant.helpers.state import state_as_number, async_reproduce_state
...
def _interpolate_states(lower_state, upper_state, position):
    ...
    for attribute in set(lower_state.attributes) & set(upper_state.attributes):
        ...
        print ("Interpolating %s between types %s and %s" % (attribute, type(lower_value), type(upper_value)))
        print ("Interpolating between %s and %s" % (lower_value, upper_value))
